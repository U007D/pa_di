...
mod pa_di;
use crate::{
    impls::log::{stdout, Log},
    traits::logger::Constructor,
    pa_di::PaDi,
};
pub use error::{Error, Result};
use teloc::{ServiceProvider, Resolver};

pub fn lib_main() -> Result<()> {
    let log = Log::new(stdout())?;
    let service_container = ServiceProvider::new().add_instance(log);

    let padi = PaDi::new(service_container);
